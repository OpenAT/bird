...
// odoo state data as a dictionary with the filtered state name as key
// (json) (state name, computed total garden_size, thumbnail ids)
var odoo_state_data_by_name = null;
var odoo_state_data_by_id = null;

// odoo community data as a dictionary with the filtered community name as key
// (json) (community name, computed total garden_size, thumbnail ids)
var odoo_community_data_by_name = null;
var odoo_community_data_by_id = null;

// List with "filtered community names" and computed centers from the austrian geojson map stored in the Java Script Variable austriaGeoJsonMin
...
// For the gallery and the last selected map feature (geo-json-feature)
var gardenMapGalleryData;
var gardenMapLastHighlightFeature;

// TODO: !!! Rewrite all of this for bird_birdwatch !!!
$(document).ready(function () {
        ...
        if ( $( "#bird_birdwatch_map" ).length ) {
            $.ajax({
                ...
                success: function (data) {
                    ...
                    // Create a dictionary from the odoo state data with the filtered state-name as keys
                    odoo_state_data_by_name = dataToMap(data.result.state_data[0], 'cmp_state', null, );
                    // HINT: This is only for the image gallery
                    odoo_state_data_by_id = dataToMap(data.result.state_data[0], 'cmp_state_id', null, null);

                    // Create a dictionary from the odoo community data with the filtered community-name as keys
                    odoo_community_data_by_name = dataToMap(data.result.community_data[0], 'cmp_community', null);
                    // HINT: This is only for the image gallery
                    odoo_community_data_by_id = dataToMap(data.result.community_data[0], 'cmp_community_code', null, null);
    ...
    //-------------------------------------
    function dataToMap(data, name, value, filter_key=true) {
        return _.reduce(data, function(acc, item) {
            if (value === null ) {
                if (filter_key) {
                    acc[filterName(item[name])] = item;
                } else {
                    acc[String(item[name])] = item;
                }
            }
            else {
                if (filter_key) {
                    ...
                    acc[String(item[name])] = item[value];
                }
            }
            return acc;
        }, {});
    ...
    // Filter the odoo state an community names to match the geojson names!
    function filterName(str) {
        var filtered = replaceUmlaute(str);
        //return filtered.replace(/\./g, '').replace(/\-/g, '').replace(/\ /g, '').toLowerCase();
        return filtered.toLowerCase().replace(/st\./g, 'sankt').replace(/\./g, '').replace(/-/g, '').replace(/ /g, '').replace(/gemeindebezirk/g, 'wien');
    }
    ...
    function initMap() {
        ...
        //console.log('initMap');
        //console.log('odoo_community_data_by_name', odoo_community_data_by_name);
        ...
        stateLayer = L.geoJson(austriaGeoJsonMin, {
        ...
        communityLayer = L.geoJson(austriaGeoJsonMin, {
        ...
        gardenMap.on('zoomend', function () {
            ...
            var zoom_level_switch = 11

            // Display states
            if (zoomLevel < zoom_level_switch) {
            ...
            // Display communities
            } else if (zoomLevel >= zoom_level_switch) {
    ...
    function stateStyle(feature) {
        ...
        if (featureStateName in odoo_state_data_by_name) {
            gsp = odoo_state_data_by_name[featureStateName].garden_size_peg;
    ...
    function communityStyle(feature) {
        ...
        if (featureCommunityName in odoo_community_data_by_name) {
            gsp = odoo_community_data_by_name[featureCommunityName].garden_size_peg;
            if (gsp > 0 && gsp <= 0.15) {
                opacity = 0.3;
                fill = '#9ee436';
            } else if (gsp > 0.15 && gsp <= 0.3 ) {
    ...
    function communityOnEachFeature(feature, layer) {
        ...
        if (community_map_name_filtered in odoo_community_data_by_name) {
                var singleObj = {};
    ...
    function createStateMarkerLayerGroup() {
        ...
        var stateMarkers = [];
        for (var stateName in odoo_state_data_by_name) {
            if (odoo_state_data_by_name[stateName].thumbnail_record_ids) {
                if (stateName in stateCenters) {
                    // create a new marker
                    var marker = L.marker(stateCenters[stateName], {
                        icon: L.divIcon({
                            html: '<img id="' + odoo_state_data_by_name[stateName].cmp_state_id + " bundesland" + '" class="gardenMapMarkerIcon" src="/gl2k_gardenvis/static/src/img/CameraIcon.png" onclick="showGardenMapGallery(this)">',
                        }),
                    });
                    // add it to the list
    ...
    function createCommunityMarkerLayerGroup() {
        ...
        for (var cName in odoo_community_data_by_name) {
            // Compute the Center
            try {
                // Get all odoo data of the community record
                var community = odoo_community_data_by_name[cName];
    ...
    function showInfoBox() {
        var maxGardenSize = 0;
        for (var key in odoo_state_data_by_name) {
            maxGardenSize = maxGardenSize + odoo_state_data_by_name[key].garden_size;
        }
